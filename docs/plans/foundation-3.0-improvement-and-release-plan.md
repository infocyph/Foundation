...
**Status:** Foundation 3.0 release plan complete for the declared support scope. All required Foundation batches and release gates are closed; Infbyte handoff remains explicitly deferred.  
**Reviewed:** 2026-09-24.  
...
| **F** | CI reproducibility + documentation | **DONE** | Reproducible matrix/services/consumer gates, immutable PHPForge workflow identity, release notes, documentation path and committed evidence index are implemented. Workflow #1723 at candidate `e91c35a` passed PHP 8.4/8.5 stable+lowest QA, both analyses, both benchmarks, clean install, all four isolated production consumers and the corrected release-evidence artifact job. |
...
Evidence is split intentionally: concise committed records live under `docs/evidence/foundation-3.0/`, while large logs, benchmark output and the generated release-evidence manifest are retained as CI artifacts. Record source SHA, Composer package references, tooling/workflow identities, runtime versions, command, result and artifact location for each gate.

## 13. Foundation 3.0 release acceptance checklist

### Required before tagging Foundation

- [x] P0 fixes and the two dependency-contract failures are closed.
- [x] Required runtime/support rows are implemented and verified, or the release's support statement is explicitly narrowed before publication.
- [x] Plain minimal production installation and optional capability isolation pass without development packages.
- [x] Config/catalog/CLI/docs agree, including built-in capabilities and core cache.
- [x] Auth/session negative paths, cleanup precedence and persistent isolation pass.
- [x] Schema migration and rolling-generation compatibility are verified for supported persistence backends.
- [x] Release build, activation, trust validation, drain, rollback and artifact retention rehearsal pass.
- [x] Representative performance/soak gates pass with interpretable evidence for the declared 3.0 support rows; native host certification remains explicitly deferred per F30-06.
- [x] PHPForge-required implementation processing/checks are run on code changes; detailed tests and the final release guard pass without weakening scope or thresholds.
- [x] PHP 8.4/8.5 stable/lowest CI, analysis, audit and clean-install jobs validate the exact final implementation/dependency revision.
- [x] Isolated Foundation consumers pass against that revision. Infbyte migration and consumer certification remain explicitly deferred to the separate skeleton task.
- [x] Migration guide, release notes and support matrix identify breaking changes and known limitations.
- [x] Every unfinished P2 item is explicitly deferred; no unfinished required gate is marked complete because historical plans were green.

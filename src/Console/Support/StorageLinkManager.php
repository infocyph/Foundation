...
final readonly class StorageLinkManager
{
    ...
    public function create(): array
    {
        ...
        if (!is_array($configured) || $configured === []) {
            throw new \RuntimeException(
                'No filesystem.links are configured; run "php infbyte module:install filesystem" first.',
            );
        }
    ...
    private function absolute(string $path): string
    {
        ...
        return $this->application->basePath(trim($path, '/\\'));
    }
    ...
    private function preparePaths(string $link, string $target, string $storage): array
    {
        ...
        return [
            $parent . DIRECTORY_SEPARATOR . basename($link),

...
final readonly class ArtifactGenerator
{
    ...
    private const array ARTIFACTS = [
        ...
        'repository' => [
            ...
            'requires' => \Infocyph\DBLayer\Query\Repository::class,
        ...
        'trait' => [
            'directory' => 'app/Concerns',
            'namespace' => 'App\\Concerns',
            'suffix' => 'Trait',
            'stub' => 'trait.stub',
        ],
        'worker' => [
            'directory' => 'app/Console/Workers',
    ...
    public function create(
        ...
        $definition = self::ARTIFACTS[$artifact] ?? throw new \InvalidArgumentException(sprintf(
        ...
        if (
            $definition['suffix'] !== ''
            && str_ends_with($class, $definition['suffix'] . $definition['suffix'])
        ) {
        ...
        $relativeDirectory = implode('/', array_filter([
            $definition['directory'],
            implode('/', $segments),
        ]));
        $path = $this->application->basePath($relativeDirectory . '/' . $class . '.php');
        ...
        return [
            'class' => implode('\\', array_filter([$namespace, $class])),
            'path' => $path,
        ];
    }
    ...
    private function commandName(string $class, array $parents): string
    {
        ...
        $segments = [...$parents, $class];

        return implode(':', array_map(
            static fn(string $segment): string => strtolower(
                preg_replace('/(?<!^)[A-Z]/', '-$0', $segment) ?? $segment,
            ),
            $segments,
        ));
    }
    ...
    private function existingAncestor(string $directory): string
    {
    ...
    private function render(
        ...
        $path = dirname(__DIR__, 3) . '/resources/stubs/create/' . $stub;
    ...
    private function segments(string $name): array
    {
        ...
        $rawSegments = explode('/', str_replace('\\', '/', trim($name)));
        $segments = [];
        foreach ($rawSegments as $segment) {
            ...
            if ($words === false) {
                throw new \InvalidArgumentException('Artifact names must contain valid PHP class segments.');
            }
            ...
            if (preg_match('/^[A-Za-z_][A-Za-z0-9_]*$/D', $normalized) !== 1) {
                throw new \InvalidArgumentException(sprintf(
                    'Artifact name segment "%s" is not a valid PHP class segment.',
                    $segment,
                ));
            }
            $segments[] = $normalized;
        ...
        if ($configured !== null && $configured !== '') {
            if (preg_match('/^[A-Za-z_][A-Za-z0-9_]*(?:\.[A-Za-z_][A-Za-z0-9_]*)*$/D', $configured) !== 1) {
                throw new \InvalidArgumentException(
                    'Repository tables must be identifiers such as users or reporting.daily_totals.',
                );
            }
    ...
    private function write(string $path, string $contents, bool $force): void
    {
        ...
        $this->assertInsideProject($this->existingAncestor($directory));

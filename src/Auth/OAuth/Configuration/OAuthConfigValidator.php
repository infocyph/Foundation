...
final readonly class OAuthConfigValidator
{
    ...
    /** @param list<ConfigIssue> $issues */
    private function validateOpenId(array &$issues): void
    {
        $enabled = $this->config->get('auth.oauth.oidc.enabled', false);
        if (!is_bool($enabled)) {
            $issues[] = new ConfigIssue('auth.oauth.oidc.enabled must be a boolean.', 'auth.oauth.oidc.enabled');
        ...
        foreach ([
            ...
            if (!is_array($values)
                ...
                || $values === []
                || !in_array($required, $values, true)
                || count($values) !== count(array_unique($values, SORT_STRING))
                || array_any($values, static fn(mixed $value): bool => !is_string($value) || $value === '')
            ) {
                $issues[] = new ConfigIssue(
        ...
        foreach ($public as $entry) {
            if (!is_array($entry)) {
                $issues[] = new ConfigIssue('OpenID public-key entries must be maps.', 'auth.oauth.oidc.signing.public_keys');
            ...
            if ($status === 'active') {
                ++$activeCount;
                if (!is_string($activeId) || !hash_equals($activeId, $id)) {
                    $issues[] = new ConfigIssue(
                        'OpenID active public key must match active_key_id.',
                        'auth.oauth.oidc.signing.public_keys',
                    );
                }
            }

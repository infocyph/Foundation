...
final readonly class WorkerManager
{
    ...
    public function run(string $name, string $routes = 'routes/workers.php'): ?int
    {
        ...
        if (!$provider instanceof WorkerProvider) {
            throw new \UnexpectedValueException(sprintf('Worker provider "%s" is invalid.', $providerClass));
        }

        $runtime = new WorkerRuntime($this->application);
        $leaseSeconds = max(30.0, (float) $this->application->config()->get('worker.lock_lease_seconds', 300.0));
